...
def update_user_profile(
    *,
    ...
    sql = f"UPDATE users SET {', '.join(updates)} WHERE id=%s AND deleted_at IS NULL"
    try:
        with mysql_conn(cfg) as conn:
            with conn.cursor() as cur:
...
def create_session(*, user_id: int, cfg: Optional[MySQLConfig] = None) -> Dict[str, Any]:
    ...
    with mysql_conn(cfg) as conn:
        with conn.cursor() as cur:
            # 取消该用户的其他活跃会话
            cur.execute("UPDATE sessions SET is_active=0 WHERE user_id=%s AND deleted_at IS NULL", (user_id,))
            cur.execute(
...
def set_active_session(*, user_id: int, session_id: str, cfg: Optional[MySQLConfig] = None) -> bool:
    with mysql_conn(cfg) as conn:
        with conn.cursor() as cur:
            cur.execute("UPDATE sessions SET is_active=0 WHERE user_id=%s AND deleted_at IS NULL", (user_id,))
            cur.execute(
                "UPDATE sessions SET is_active=1 WHERE user_id=%s AND session_id=%s AND deleted_at IS NULL",
                (user_id, session_id),
            )
            return cur.rowcount > 0
...
def update_session_title(
    *,
    ...
    with mysql_conn(cfg) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE sessions SET title=%s WHERE user_id=%s AND session_id=%s AND deleted_at IS NULL",
                (title, user_id, session_id),
            )
            return cur.rowcount > 0


def soft_delete_session(*, user_id: int, session_id: str, cfg: Optional[MySQLConfig] = None) -> bool:
    with mysql_conn(cfg) as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE sessions SET deleted_at=NOW(), is_active=0 WHERE user_id=%s AND session_id=%s AND deleted_at IS NULL",
                (user_id, session_id),
            )
            affected = cur.rowcount > 0
            cur.execute(
                "UPDATE messages SET deleted_at=NOW() WHERE session_id=%s AND deleted_at IS NULL",
                (session_id,),
            )
            return affected
...
def list_messages(*, user_id: int, session_id: str, cfg: Optional[MySQLConfig] = None) -> List[Dict[str, Any]]:
    ...
    WHERE s.user_id=%s AND s.session_id=%s AND s.deleted_at IS NULL
    ...
    *,
    ...
    with mysql_conn(cfg) as conn:
        with conn.cursor() as cur:
            ...
            # bump updated_at by touching session row
            cur.execute(
                "UPDATE sessions SET updated_at=NOW() WHERE user_id=%s AND session_id=%s AND deleted_at IS NULL",
                (user_id, session_id),
            )
            return inserted
