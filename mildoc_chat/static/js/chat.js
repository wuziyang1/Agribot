...
// 将模型返回内容按 Markdown 渲染到指定元素中（可选渲染公式）
    ...
    while (true) {
      ...
      while ((index = buffer.indexOf('\n')) >= 0) {
        ...
        if (msg.type === 'chunk' && msg.data && typeof msg.data.content === 'string') {
          // 流式阶段：只做纯文本追加，避免每个 chunk 都 Markdown/MathJax 重渲染导致卡顿和空行
