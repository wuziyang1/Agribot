...
from agribot_chat.routers.logging_utils import setup_logging
...
from agribot_chat.routers.login_flask import login_bp
from agribot_chat.routers.register_flask import register_bp
...
from agribot_chat.forgot_password_smtp import send_reset_code, reset_password
...
app.secret_key = os.getenv("AGRIBOT_CHAT_SECRET_KEY", "change-me-in-production")
...
def _require_login():
    ...
    exempt_endpoints = {
        "health",
        "agribot_chat_login.login",
        "agribot_chat_login.login_post",
        "agribot_chat_login.logout",
        "agribot_chat_register.register",
        "agribot_chat_register.register_post",
        "agribot_chat_register.api_register_send_code",
...
def index():
    """简易 ChatGPT 风格网页"""
    username = session.get("chat_username") or "Agribot 用户"
        ...
        return jsonify(
            {
                "success": False,
                "content": "",
                "error_message": "RAG 服务初始化失败，请检查 agribot_chat 的 .env 配置。",
                "source_documents": [],
                "token_usage": None,
            }
        )
...
if __name__ == "__main__":
    log_path = os.getenv("AGRIBOT_CHAT_LOG", os.path.join(os.path.dirname(__file__), "agribot_chat.log"))
    setup_logging(log_path=log_path, level=logging.INFO)

    host = os.getenv("AGRIBOT_CHAT_HOST", "0.0.0.0")
    port = int(os.getenv("AGRIBOT_CHAT_PORT", "8890"))
    debug = os.getenv("AGRIBOT_CHAT_DEBUG", "False").lower() == "true"

    logger.info("Agribot Chat Starting...")
