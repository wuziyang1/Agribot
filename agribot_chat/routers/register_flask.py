...
from agribot_chat.routers.database import create_user, get_user_by_email, get_user_by_username

logger = logging.getLogger(__name__)

register_bp = Blueprint("agribot_chat_register", __name__)
...
def send_email_code(email: str, code: str, *, scene: str = "register") -> Tuple[bool, str]:
    ...
    smtp_host = (os.getenv("SMTP_SERVER") or os.getenv("AGRIBOT_CHAT_SMTP_HOST") or "").strip()
    smtp_port = int(os.getenv("SMTP_PORT") or os.getenv("AGRIBOT_CHAT_SMTP_PORT") or "587")
    smtp_user = (os.getenv("SMTP_USERNAME") or os.getenv("AGRIBOT_CHAT_SMTP_USER") or "").strip()
    smtp_password = (os.getenv("SMTP_PASSWORD") or os.getenv("AGRIBOT_CHAT_SMTP_PASSWORD") or "").strip()
    smtp_from = (
        os.getenv("FROM_EMAIL")
        or os.getenv("AGRIBOT_CHAT_SMTP_FROM")
        or smtp_user
    ).strip()
    ...
    try:
        msg = EmailMessage()
        subject = "Agribot Chat 注册验证码" if scene == "register" else "Agribot Chat 找回密码验证码"
        msg["Subject"] = subject
        msg["From"] = smtp_from
        msg["To"] = email
        msg.set_content(
            f"你的 Agribot Chat { '注册' if scene == 'register' else '找回密码' }验证码是：{code}\n\n有效期 10 分钟。"
        )
